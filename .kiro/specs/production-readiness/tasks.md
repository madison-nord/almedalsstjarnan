...
- [ ] 1. Quick wins: rename, pin dependency, remove tabs permission
  - [ ] 1.1 Rename `sha256Hex` to `fnv1aHex` in `src/core/event-normalizer.ts`
  ...
  - [ ] 1.2 Pin `sharp` to exact version in `package.json`
    - Change `"sharp": "^0.34.5"` to `"sharp": "0.34.5"` (remove caret)
    - _Requirements: 10.1, 10.2_

  - [ ] 1.3 Remove `tabs` permission from `src/extension/manifest/base.json`
    - Remove `"tabs"` from the `permissions` array, leaving `["storage", "downloads"]`
    - _Requirements: 2.1, 2.2, 2.3_

  - [ ] 1.4 Add developer loading instructions to `README.md`
