...
describe('Property 4: Decorative Icon Accessibility', () => {
  ...
  const groupIndices = Array.from({ length: HELP_FEATURE_GROUPS.length }, (_, i) => i);

  it('every SVG icon at a randomly selected feature group index has aria-hidden="true"', () => {
    const adapter = setupAdapter();

    fc.assert(
      fc.property(fc.constantFrom(...groupIndices), (index) => {
  ...
  it('ALL SVG elements in the modal have aria-hidden="true"', () => {
    ...
    fc.assert(
      fc.property(fc.constantFrom('popup' as const, 'page' as const), (layoutMode) => {
        const { container } = render(
          React.createElement(HelpModal, {
            adapter,
            onDismiss: vi.fn(),
